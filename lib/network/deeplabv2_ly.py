...
class DeeplabV2(ResNet):
    def __init__(self, n_class, block, layers, pyramids):
        ...
        self.bn1 = InstanceWhitening(self.inplanes)
    ...
    def forward(self, input, input_j=None, gts=None, aux_gts=None):
        k_arr = []
        q_arr = []
        ...
        if input_j is not None:
            ...
            x1_j = self.aspp1(x_j_u)
            x2_j = self.aspp2(x_j_u)
            x3_j = self.aspp3(x_j_u)
            x4_j = self.aspp4(x_j_u)

            x_j = x1_j + x2_j + x3_j + x4_j
            x_j = F.upsample(x_j, size=input_j.size()[2:], mode='bilinear', align_corners=True)
            return x, x_j, aux_out, aux_gts, k_arr, q_arr
        return x
