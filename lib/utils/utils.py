...
def adjust_learning_rate(optimizer, i_iter, trainSteps, args):
    lr = lr_poly(args.lr, i_iter, trainSteps)
...
def get_confusion_matrix(label, pred, size, num_class, ignore=-1):
    ...
    seg_gt = np.asarray(
    label.cpu().numpy()[:, :size[-2], :size[-1]], dtype=np.int8)
    ignore_index = seg_gt != ignore
    seg_gt = seg_gt[ignore_index].astype('int32')
    seg_pred = seg_pred[ignore_index].astype('int32')
    # print(seg_gt, seg_pred, num_class)
    # print(seg_gt * num_class + seg_pred)
    index = (seg_gt * num_class + seg_pred).astype('int32')
    # print(index)
    label_count = np.bincount(index)
    confusion_matrix = np.zeros((num_class, num_class))
...
def validate(args, testloader, model, CL=False, CCDR=False, Blind=False, save=None, data_name='WHU'):
    model.eval()
    nums = 1
    confusion_matrix = np.zeros((args.num_classes, args.num_classes, nums))
    with torch.no_grad():
        ...
        for batch in tqdm(testloader):
            ...
            image = image.cuda()
            label = label.long().cuda()
            # print('label', label.shape, label.unique())
            try:
                if CCDR is True:
                    pred, _ = model(image)
                elif CL is False:
            ...
            if save != None:
                if data_name == 'WHU':
                    pred_ = torch.squeeze(torch.argmax(pred, dim=1)).cpu().numpy()
                    name = name[0] + '.tif'
                    # print('pred_', np.unique(pred_))
                    label_color = label2image(pred_, 'Satellite2')
                    tifffile.imwrite(os.path.join(save, name), label_color)
                elif data_name == 'OM':
                    pred_ = torch.squeeze(torch.argmax(pred, dim=1)).cpu().numpy()
                    name = name[0] + '.png'
                    # print('pred_', np.unique(pred_))
                    label_color = label2image(pred_, 'postdam')
                    label_color.astype(np.uint8)
                    image = Image.fromarray(label_color, mode='RGB')
                    image.save(os.path.join(save, name))
                    # cv2.imwrite(os.path.join(save, name), label_color, [cv2.IMWRITE_PNG_COMPRESSION, 9])
    ...
    if args.num_classes != 2:
        return mean_IoU, IoU_array, PA, f1_array, f1
    return mean_IoU, IoU_array, PA, f1_array[-1], f1 #, f1_array[0] #, f1_array, recall_array, precision_array
