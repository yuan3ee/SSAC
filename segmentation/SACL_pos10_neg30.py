...
def region_contrast_loss(num_class, keys, vals, region_memory, device):
    ...
    for cls_ind in range(num_class):
        if cls_ind in vals:
            ...
            memory_bank = region_memory['region_class' + str(cls_ind)].clone().detach()  # [256, N]
            num_samples = memory_bank.size(1)
            num_select = min(10, num_samples)
            perm = torch.randperm(num_samples)[:num_select]  # 随机选列索引
            sampled_pos = memory_bank[:, perm].T  # 先选列，再转置成 [num_select, 256]
            query = query.unsqueeze(0)  # [1, 256]
            l_pos = query * sampled_pos  # [num_select, 256]，广播正常

            # query = query.unsqueeze(0)
            # print('region_memory',region_memory['region_class' + str(cls_ind)].shape)
            #
            # print('query', query.shape, 'sampled_pos',sampled_pos.shape)
            ...
            for cls_ind2 in tmp:
                try:
                    memory_bank_neg = region_memory['region_class' + str(cls_ind2)].clone().detach()  # [256, N_neg]
                    num_samples_neg = memory_bank_neg.size(1)
                    num_select_neg = min(300, num_samples_neg)
                    perm_neg = torch.randperm(num_samples_neg)[:num_select_neg]
                    sampled_neg = memory_bank_neg[:, perm_neg].T  # [num_select_neg, 256]
                    l_neg += query * sampled_neg
                except:
                    memory_bank_neg = region_memory['region_class' + str(cls_ind2)].clone().detach()  # [256, N_neg]
                    num_samples_neg = memory_bank_neg.size(1)
                    num_select_neg = min(300, num_samples_neg)
                    perm_neg = torch.randperm(num_samples_neg)[:num_select_neg]
                    sampled_neg = memory_bank_neg[:, perm_neg].T  # [num_select_neg, 256]
                    new = query.unsqueeze(1) * sampled_neg  # [B, num_neg, C]
                    if new.shape[-1] < l_neg.shape[-1]:
                        dims = l_neg.shape[-1] - new.shape[-1]
                        new = torch.cat((new, torch.zeros(size=[new.shape[0], new.shape[1], dims]).cuda()), dim=-1)
                        l_neg += new
                    elif new.shape[-1] > l_neg.shape[-1]:
                        dims = new.shape[-1] - l_neg.shape[-1]
                        l_neg = torch.cat((l_neg, torch.zeros(size=[l_neg.shape[0], l_neg.shape[1], dims]).cuda()),
                                          dim=-1)
                        l_neg += new
