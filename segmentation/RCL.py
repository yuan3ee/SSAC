...
for i_epoch in range(1, epoch + 1):
    RC_memory = region_memory(num_class=num_classes, device=device) # construct a feature memory
    for img, label in train_loader:
        pred, feature, feature_pred = model(img)
        keys, vals = computer_region_center(feature, feature_pred, num_class=num_classes)
        for class_i in range(num_classes):
            RC_memory = update_region_memory(RC_memory, keys, vals, class_i)
        if i_epoch == 1:
            loss_rcl = region_contrast_loss(num_classes, keys, vals, RC_memory, device)
        else:
            loss_rcl = region_contrast_loss(num_classes, keys, vals, RC_memory_old, device)
    RC_memory_old = RC_memory